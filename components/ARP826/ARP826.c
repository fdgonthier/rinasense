 ...
 * */
bool_t xARPResolveGPA(const gpa_t *pxSpa, const gpa_t *pxTpa, const gha_t *pxSha);
...
bool_t xARPAddressGPAShrink(gpa_t *pxGpa, uint8_t ucFiller)
{
	string_t pucNewAddress;
	string_t pucPosition;
	size_t uxLength;
...
bool_t xARPResolveGPA(const gpa_t *pxTpa, const gpa_t *pxSpa, const gha_t *pxSha)
{
...
void vARPInitCache(void)
{
	...
	gpa_t *pxProtocolAddress = pxCreateGPA((uint8_t *)pcAd, 5);
