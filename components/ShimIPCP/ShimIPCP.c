...
struct ipcpFactoryData
{
	RsList_t xInstancesShimWifiList;
};

static struct ipcpFactoryData xFactoryShimWifiData;
...
static shimFlow_t *prvShimFindFlow(struct ipcpInstanceData *pxData);

static shimFlow_t *prvShimFindFlowByPortId(struct ipcpInstanceData *pxData, portId_t xPortId);

/* @brief Destroy an specific Flow */
static bool_t prvShimFlowDestroy(struct ipcpInstanceData *xData, shimFlow_t *xFlow);

/* @brief Unbind and Destroy a Flow*/
static bool_t prvShimUnbindDestroyFlow(struct ipcpInstanceData *xData, shimFlow_t *xFlow);
...
bool_t xShimFlowDeallocate(struct ipcpInstanceData *xData, portId_t xId)
{
	shimFlow_t *xFlow;
...
bool_t xShimFlowAllocateRequest(portId_t xPortId,
                                ...
                                struct ipcpInstanceData *pxData)
{
...

{
	...
	shimFlow_t *pxFlow;
	struct ipcpInstance *pxShimIpcp;
    struct timespec ts;
...
void vShimNameFini(name_t *pxName)
{
	...
	{
		vRsMemFree(pxName->pcProcessName);
		pxName->pcProcessName = NULL;
	}
	if (pxName->pcProcessInstance)
	{
		vRsMemFree(pxName->pcProcessInstance);
		pxName->pcProcessInstance = NULL;
	}
	if (pxName->pcEntityName)
	{
		vRsMemFree(pxName->pcEntityName);
		pxName->pcEntityName = NULL;
	}
	if (pxName->pcEntityInstance)
	{
		vRsMemFree(pxName->pcEntityInstance);
		pxName->pcEntityInstance = NULL;
	}

	LOGI(TAG_SHIM, "Name at %pK finalized successfully", pxName);
}

static bool_t prvShimUnbindDestroyFlow(struct ipcpInstanceData *xData,
										   shimFlow_t *xFlow)
{
...
bool_t xShimSDUWrite(struct ipcpInstanceData *pxData, portId_t xId, struct du_t *pxDu, bool_t uxBlocking)
{
	...
	if (uxLength > MTU)
	{
		LOGE(TAG_SHIM, "SDU too large (%lu), dropping", uxLength);
		xDuDestroy(pxDu);
		return false;
	}
	...
	*/
    ...
    if (xSendEventStructToIPCPTask(&xTxEvent, &ts) == false) {
        LOGE(TAG_WIFI, "Failed to enqueue packet to network stack %p, len %d", pxNetworkBuffer, pxNetworkBuffer->xDataLength);
        vReleaseNetworkBufferAndDescriptor(pxNetworkBuffer);
        return false;
    }
...
static struct ipcpInstanceOps xShimWifiOps = {
...
{
	struct ipcpInstance *pxInst;
	struct ipcpInstanceData *pxInstData;
	struct flowSpec_t *pxFspec;
	name_t *pxName;
	MACAddress_t *pxPhyDev;
...
{
	LOGI(TAG_SHIM, "Registering Shim WiFi Factory");

	if (xFactoryIPCPRegister(pxFactoriesList, eShimWiFi, &xFactoryShimWifiData, &xFactoryShimWifiOps))
	{
		return true;
	}
	return false;
