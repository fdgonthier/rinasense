...
static bool_t xEfcpDestroy(struct efcp_t * pxInstance)
{
        ...
        if (pxInstance->pxConnection)
        {
                ...
                if (is_cep_id_ok(pxInstance->pxConnection->xSourceCepId)) {
                        RsAssert(pxInstance->pxContainer);
...
struct efcpContainer_t *pxEfcpContainerCreate(void)
{
        ...
        pxContainer = pvRsMemAlloc(sizeof(*pxContainer));

        if (!pxEfcpContainer)
                return NULL;

        pxContainer->pxCidm = numMgrCreate(MAX_CEP_ID);
        ...
        LOGI(TAG_EFCP, "EFCP container instance %p created", pxContainer);
