...
gpa_t *pxNameToGPA(const name_t *pcName)
{
	...
	pxGpa = pxCreateGPA((uint8_t *)(pcTmp), strlen(pcTmp)); //considering the null terminated
...
uint8_t *pucCreateAddress(size_t uxLength)
{
	...
	pucAddress = (uint8_t *)pvRsMemAlloc(uxLength);
    memset(pucAddress, 0, uxLength);

	return pucAddress;
}

gpa_t *pxCreateGPA(const string_t pucAddress, size_t uxLength)
{
	gpa_t *pxGPA;
